...
module.exports = app => {
	...
	return {
		...
		createTable(modelData) {
			...
			const { ifNotExist, databaseName, items, options } = modelData;
		...
		createCustomIndex(modelData) {
			...
			return assignTemplates(templates.createCustomIndex, {
				ifNotExistStatement,
				name: `"${name}"`,
				tableName,
				column,
		...
		createSearchIndex(modelData) {
			...
			return assignTemplates(templates.createSearchIndex, {
				ifNotExistStatement,
				tableName,
				columnsStatement,
				profiles,
		...
		dropType(modelData) {
			...
			return assignTemplates(templates.dropType, {
				...
				typeName: wrapInQuotes(typeName),
			});
		},
		...
		renameColumn(modelData) {
			...
			return assignTemplates(templates.renameColumn, {
				alterTablePrefixStatement,
				oldName: wrapInQuotes(columnData.oldName),
				newName: wrapInQuotes(columnData.newName),
			});
		},
		...
		addEntity(modelData) {
			...
			return assignTemplates(templates.addEntity, {
				alterTablePrefixStatement,
				name: wrapInQuotes(columnData.name),
		...
		deleteEntity(modelData) {
			...
			return assignTemplates(templates.deleteEntity, {
				alterTablePrefixStatement,
				name: wrapInQuotes(columnData.name),
			});
		},
		...
		addPropertyToUdt(modelData) {
			...
			return assignTemplates(templates.addPropertyToUdt, {
		...
		createUdt(modelData) {
			...
			return assignTemplates(templates.createUdt, {
				keySpaceName: wrapInQuotes(keySpaceName),
				udtName: wrapInQuotes(udtName),
				columnScript: tab(columnScript),
			});
		},
		...
		updateUdtType(modelData) {
			...
			return assignTemplates(templates.updateUdtType, {
				alterTypePrefixStatement,
				udtName: wrapInQuotes(udtName),
				name: wrapInQuotes(columnData.name),
				type: columnData.type,
		...
		updateType(modelData) {
			...
			return assignTemplates(templates.updateType, {
				alterTablePrefixStatement,
				name: wrapInQuotes(columnData.name),
				type: columnData.type,
			});
		...
		updateTableOptions(compMod, tableName, isGetOptionScript) {
			...
			return optionsStatement
				? assignTemplates(templates.updateTableOptions, {
						alterTablePrefixStatement,
						optionsStatement: tab(optionsStatement),
					})
				: '';
		},
		...
		alterKeySpaceReplication(keySpaceName, replication, durableWrites) {
			return assignTemplates(templates.alterKeySpaceReplication, {
				keySpaceName: wrapInQuotes(keySpaceName),
				replication: tab(replication),
				durableWrites: tab(durableWrites, 2),
			});
		},
